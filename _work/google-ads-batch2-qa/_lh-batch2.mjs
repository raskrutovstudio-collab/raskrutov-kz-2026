...
const origin = process.env.LH_ORIGIN || 'http://127.0.0.1:8777';
const slugs = ['aktau', 'atyrau', 'kokshetau', 'kostanay'];
const outFile = path.resolve('site_mirror/_work/google-ads-batch2-qa/lighthouse.json');
const results = {};

function runOnce(url, i) {
  const out = path.join(os.tmpdir(), `gads-b2-lh-${Date.now()}-${i}.json`);
  const args = [
    lhCli,
    url,
    '--quiet',
  ...
  return {
    ...
    lcpElement: audits['largest-contentful-paint-element']?.displayValue || audits['largest-contentful-paint']?.displayValue || '',
  };
}
