...
const outDir = path.resolve('site_mirror/_work/google-ads-batch2-qa');
fs.mkdirSync(outDir, { recursive: true });

const cities = [
  { slug: 'aktau', name: 'aktau' },
  { slug: 'atyrau', name: 'atyrau' },
  { slug: 'kokshetau', name: 'kokshetau' },
  { slug: 'kostanay', name: 'kostanay' },
];
...
try {
  for (const city of cities) {
    ...
    const widths = city.slug === 'aktau' ? [390, 430, 768, 1440] : [390, 1440];
...
for (const c of report) {
  for (const [w, v] of Object.entries(c.viewports)) {
    const fail = v.geo.issues.length || v.geo.hScroll > 1 || v.geo.scopeIcons < v.geo.scopeItems || v.geo.taskMarks < v.geo.taskLis;
    console.log(`${c.city} ${w} hScroll=${v.geo.hScroll} icons=${v.geo.scopeIcons}/${v.geo.scopeItems} marks=${v.geo.taskMarks}/${v.geo.taskLis} issues=${v.geo.issues.length} errors=${v.consoleErrors.length} ${fail ? 'FAIL' : 'PASS'}`);
