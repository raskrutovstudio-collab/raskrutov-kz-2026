/**
 * Pass2 after a11y underline fix: 3× mobile + 1× desktop per city on gzip :8765
 */
...
function runLh(city, label, formFactor) {
  const outPath = path.join(OUT, `lh-pass2-${city}-${label}.json`);
  const args = [
    lhCli,
    urlFor(city),
    '--quiet',
  ...
  console.log(`\n=== LH pass2 ${city} ${label} (${formFactor}) ===`);
  ...
  const bpFailed = [];
  for (const ref of cats['best-practices']?.auditRefs || []) {
    const a = report.audits[ref.id];
    if (!a || typeof a.score !== 'number' || a.score >= 1) continue;
    bpFailed.push({ id: a.id, title: a.title, score: a.score });
  }
...
const batch = { measuredAt: new Date().toISOString(), pass: 'pass2-a11y-underline', cities: {} };
...
for (const city of CITIES) {
  ...
  fs.writeFileSync(
    path.join(OUT, `lh-pass2-${city}-summary.json`),
    JSON.stringify(batch.cities[city], null, 2)
  );
}

fs.writeFileSync(path.join(OUT, 'lh-pass2-all.json'), JSON.stringify(batch, null, 2));
console.log('\n=== PASS2 LH DONE ===');
console.log(
  JSON.stringify(
    Object.fromEntries(
      Object.entries(batch.cities).map(([c, v]) => [
        c,
        { median: v.mobileMedian, desktop: v.desktop.scores, gate: v.gate },
      ])
    ),
    null,
    2
