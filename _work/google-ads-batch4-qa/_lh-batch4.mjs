...
const slugs = ['taldykorgan', 'taraz', 'turkestan', 'uralsk', 'ust-kamenogorsk'];
const outFile = path.resolve('site_mirror/_work/google-ads-batch4-qa/lighthouse.json');
const results = {};

function runOnce(url, i) {
  const out = path.join(os.tmpdir(), `gads-b4-lh-${Date.now()}-${i}.json`);
  const args = [
    lhCli,
    url,
    '--quiet',
...
for (const slug of slugs) {
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    console.log(`  P=${runs[i - 1].performance} A11Y=${runs[i - 1].accessibility} SEO=${runs[i - 1].seo} LCP=${runs[i - 1].lcp} TBT=${runs[i - 1].tbt} CLS=${runs[i - 1].cls}`);
  }
  results[slug] = {
    url,
    runs,
