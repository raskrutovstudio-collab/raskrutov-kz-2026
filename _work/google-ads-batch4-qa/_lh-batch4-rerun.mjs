...
const slugs = ['taraz', 'turkestan', 'ust-kamenogorsk'];
const outFile = path.resolve('site_mirror/_work/google-ads-batch4-qa/lighthouse.json');
const results = JSON.parse(fs.readFileSync(outFile, 'utf8'));

function runOnce(url, i) {
  const out = path.join(os.tmpdir(), `gads-b4-lh2-${Date.now()}-${i}.json`);
  const args = [
    lhCli,
    url,
    '--quiet',
  ...
  if (run.status !== 0) {
    throw new Error(`lighthouse failed ${url} run ${i} status=${run.status}`);
  }
...
for (const slug of slugs) {
  ...
  const runs = [];
  for (let i = 1; i <= 3; i++) {
    console.log(`LH2 ${slug} run ${i}`);
    runs.push(runOnce(url, i));
    console.log(`  P=${runs[i - 1].performance} A11Y=${runs[i - 1].accessibility} SEO=${runs[i - 1].seo} LCP=${runs[i - 1].lcp} TBT=${runs[i - 1].tbt} CLS=${runs[i - 1].cls} ${runs[i - 1].lcpSnippet}`);
  }
  results[slug] = {
    url,
    note: 'official 3-run after removing page-local font preload',
    runs,
    median: {
  ...
  console.log(`${slug} median`, results[slug].median, 'min', results[slug].minPerformance);
}
