...
const url = (process.env.QA_ORIGIN || 'http://127.0.0.1:8791') + '/web-studiya/kontekstnaya-reklama/yandex-direct/karaganda/';
const OUT = path.resolve('site_mirror/_work/yandex-direct-regional-scale/karaganda');
const CHROME = 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe';

function run(label, extra) {
  const out = path.join(os.tmpdir(), 'lh-kar-' + label + '-' + Date.now() + '.json');
