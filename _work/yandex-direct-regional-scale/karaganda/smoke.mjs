...
await page.route('**/*', async (route) => {
  ...
  if (r.method() === 'POST') { posted.push(r.url()); return route.fulfill({ status: 200, contentType: 'application/json', body: '{"ok":true}' }); }
...
// FAQ accordion
const q1 = page.locator('#yd-kar-faq-q1');
...
out.faq_answer_visible = await page.locator('#yd-kar-faq-a1').isVisible();
await q1.click();
await page.waitForTimeout(250);
...
out.popup_form_visible = await page.locator('#rk-form-popup-yd-karaganda').isVisible().catch(() => false);

if (out.popup_form_visible) {
  out.popup_fields = await page.evaluate(() => {
    const f = document.querySelector('#rk-form-popup-yd-karaganda');
  ...
  out.popup_closed_by_esc = !(await page.locator('#rk-form-popup-yd-karaganda').isVisible().catch(() => false));
}
...
out.contacts_fields = await page.evaluate(() => {
  const f = document.querySelector('#rk-form-contacts-yd-karaganda');
  if (!f) return null;
  return {
...
await page.locator('#rk-form-contacts-yd-karaganda button[type="submit"], #rk-form-contacts-yd-karaganda [type="submit"]').first().click();
await page.waitForTimeout(500);
out.posted_on_empty_submit = posted.length;

await page.fill('#yd-kar-contact-name', 'Тест QA');
await page.fill('#yd-kar-contact-phone', '+7 (701) 234 56 78');
const agree = page.locator('#rk-form-contacts-yd-karaganda input[type="checkbox"]').first();
