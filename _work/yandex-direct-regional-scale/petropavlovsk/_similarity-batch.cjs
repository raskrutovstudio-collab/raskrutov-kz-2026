...
const peers = [
  ...
  "site_mirror/web-studiya/kontekstnaya-reklama/petropavlovsk/index.html",
  "site_mirror/web-studiya/kontekstnaya-reklama/google-ads/petropavlovsk/index.html",
];
...
for (const p of peers) {
  ...
  try {
    ...
    console.log("PARSE FAIL", p, r.stdout, r.stderr);
    process.exit(2);
  }
  const name = p.includes("/google-ads/")
    ? "google-ads/petropavlovsk"
    : p.includes("/kontekstnaya-reklama/petropavlovsk/")
      ? "kontekst/petropavlovsk"
  ...
  if (!j.pass && j.long_dup_samples) {
    console.log("  dups:", JSON.stringify(j.long_dup_samples));
  }
}
console.log("\nWORST:", JSON.stringify(worst, null, 2));
fs.writeFileSync(
  "site_mirror/_work/yandex-direct-regional-scale/petropavlovsk/_sim-out.json",
