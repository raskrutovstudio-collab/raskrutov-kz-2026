...
const url = (process.env.QA_ORIGIN || 'http://127.0.0.1:8791') + '/web-studiya/kontekstnaya-reklama/yandex-direct/aktobe/';
const OUT = path.resolve('site_mirror/_work/yandex-direct-regional-scale/aktobe');
const CHROME = 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe';

function run(label, extra) {
  const out = path.join(os.tmpdir(), 'lh-aktb-' + label + '-' + Date.now() + '.json');
